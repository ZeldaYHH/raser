...
def CreateBernoulli (device, region):
    ...
    node_in_2d.CreateEdgeModel(device, region, "vdiff", vdiffstr)
    node_in_2d.CreateEdgeModel(device, region, "vdiff:Potential@n0",  "V_t^(-1)")
    node_in_2d.CreateEdgeModel(device, region, "vdiff:Potential@n1",  "-vdiff:Potential@n0")
    node_in_2d.CreateEdgeModel(device, region, "Bern01",              "B(vdiff)")
    node_in_2d.CreateEdgeModel(device, region, "Bern01:Potential@n0", "dBdx(vdiff) * vdiff:Potential@n0")
    node_in_2d.CreateEdgeModel(device, region, "Bern01:Potential@n1", "-Bern01:Potential@n0")
...
#  Jn = "ElectronCharge*{0}*EdgeInverseLength*V_t*((Electrons@n1-Electrons@n0)*Bern01 +  Electrons@n1*vdiff)".format(mu_n)
    ...
    node_in_2d.CreateEdgeModel(device, region, "ElectronCurrent", Jn)
    for i in ("Electrons", "Potential", "Holes"):
        node_in_2d.CreateEdgeModelDerivatives(device, region, "ElectronCurrent", Jn, i)
...
#  Jp ="-ElectronCharge*{0}*EdgeInverseLength*V_t*((Holes@n1 - Holes@n0)*Bern01 - Holes@n0*vdiff)".format(mu_p)
    node_in_2d.CreateEdgeModel(device, region, "HoleCurrent", Jp)
