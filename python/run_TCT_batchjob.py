...
'''
Description:  Run TCT batch model
@Date       : 
@Author     : 
@version    : 1.0
'''
...
#        job_name=write_job(input,run_code="./python/TCTtest.py det_model=plugin3Dscan")
    print(input.name)
    if "HPK-Si-PIN" in input.name:
        job_name=write_job(input,run_code="./python/TCTtest.py det_name=HPK-Si-PIN laser_model=edge_TCT_Si ")
    elif "HPK-Si-LGAD" in input.name:
        job_name=write_job(input,run_code="./python/TCTtest.py det_name=HPK-Si-LGAD laser_model=edge_TCT_Si ")
    else:
...
def write_job(input,run_code):
    ...
    for j in range (1,input.para_number+1):
        ...
        for i in range(input.instance_number):
            ...
            laser_parfile=" laser_parfile="+os.getcwd()+"/"+path+"laser_setting%s.json "%(j)
            f1.write("#!/bin/bash \n")
            ...
            f1.write(run_code+parfile+laser_parfile)
            f1.close()
...
def modify_json(input,name):
  if input.para_name == "scan_z":
    path_file1 = "setting.json"
    with open(path_file1) as f:
         paras = json.load(f)
         for para in paras:
           if input.name in "HPK-Si-PIN" and para['det_name'] in "HPK-Si-PIN":
             for i in range(input.para_number):
                 outfile1 = name+"setting%s.json"%(i+1)
                 json_str1 = json.dumps([para], indent=4)
                 with open(outfile1,"w") as f:
                      f.write(json_str1)
                      f.close()
           elif input.name in "HPK-Si-LGAD" and para['det_name'] in "HPK-Si-LGAD":
             for i in range(input.para_number):
                 outfile1 = name+"setting%s.json"%(i+1)
                 json_str1 = json.dumps([para], indent=4)
                 with open(outfile1,"w") as f:
                      f.write(json_str1)
                      f.close()
    path_file = "laser_setting.json"
    with open(path_file) as f:
        paras = json.load(f)
        for para in paras:
            if input.name in "HPK-Si-PIN" and para['laser_model'] in "edge_TCT_Si":
                for i in range(input.para_number):
                    if input.para_name == "NO":
                        pass
                    else:
                        if input.para_name == "scan_z":
                            para.update({'fz_rel':str(0+i*0.02)})
                        else:
                            print("Select right para_name:scan_volatge,scan_doping,scan_temp,scan_thick,NO")              
                    outfile = name+"laser_setting%s.json"%(i+1)
                    json_str = json.dumps([para], indent=4)
                    with open(outfile,"w") as f:
                        f.write(json_str)
                        f.close()
            elif input.name in "HPK-Si-LGAD" and para['laser_model'] in "edge_TCT_Si":
                for i in range(input.para_number):
                    if input.para_name == "NO":
                        pass
                    else:
                ...
                pass
  elif input.para_name == "scan_voltage":
    path_file1 = "laser_setting.json"
    with open(path_file1) as f:
         paras = json.load(f)
         for para in paras:
           if para['laser_model'] in "edge_TCT_Si":
             for i in range(input.para_number):
                 outfile1 = name+"laser_setting%s.json"%(i+1)
                 json_str1 = json.dumps([para], indent=4)
                 with open(outfile1,"w") as f:
                      f.write(json_str1)
                      f.close()
                ...
                for i in range(input.para_number):
                    if input.para_name == "NO":
                        ...
                        if input.para_name == "scan_voltage":
                            para.update({'voltage':str(200-20*i)})
                        else:
                            print("Select right para_name:scan_volatge,scan_doping,scan_temp,scan_thick,NO")
