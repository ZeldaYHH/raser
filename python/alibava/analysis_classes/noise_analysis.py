...
class NoiseAnalysis:
    ...
    def mask_alibava_chips(self, chips_to_keep=(1,2), max_channels = 256):# used
        """Defines which chips should be considered"""
        final_channels = np.array([], dtype=np.int32)
        for chip in chips_to_keep:
            start = (chip-1)*128
            to_keep = np.arange(start, start+128, dtype=np.int32)
            final_channels = np.append(final_channels, to_keep)

        # No find the masked channels
        channels = np.arange(max_channels, dtype=np.int32)
        return final_channels, np.setdiff1d(channels, final_channels)


    def detect_noisy_strips(self, Noise, Noise_cut):# used
